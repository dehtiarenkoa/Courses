...
class Perceptron:
    ...
    def vectorized_forward_pass(self, input_matrix):
        ...
        #result = input_matrix @ self.forward_pass(input_matrix)
        #my_vec = np.array(self.forward_pass((for raw in input_matrix)))
        #return  my_vec

        mm=np.array([[self.forward_pass(raw) for raw in input_matrix]]).T
        #print(mm.shape)
# ???? it works, but how?????
        return mm
    ...
    def train_on_single_example(self, example, y):
        ...
        ## Этот метод необходимо реализовать
        s = sum(self.w * example, self.b)
        predicted = 1 if s > 0 else 0
        error = y - predicted
...
w = np.array([7, 22, 13])
y = 0
example = np.array([4, 12, 33])
s = sum(w * example)

predict = int(np.dot(w.T, example)+5)>0
error = y-predict
print(predict)
print(w * example)
print(s)
